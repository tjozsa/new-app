        ...
        stage('Publish Artifact') {
            steps {
                withEnv(["ARTIFACTORY_SERVER_URL=${env.ARTIFACTORY_SERVER}", "ARTIFACTORY_LOGIN=admin", "ARTIFACTORY_PASSWORD=password", "ARTIFACT=spincast-todobackend-inmemory-1.0.2.jar"]) {
                    sh 'curl -u ${ARTIFACTORY_LOGIN}:${ARTIFACTORY_PASSWORD} -X PUT "${ARTIFACTORY_SERVER_URL}/artifactory/todobackend-java/libs-snapshot-local/${ARTIFACT}" -T /var/jenkins_home/workspace/todobackend-java/spincast-todobackend-inmemory/target/${ARTIFACT}'
                }
            }
        }
        stage('Deploy') {
